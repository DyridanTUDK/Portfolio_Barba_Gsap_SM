...
function animateSlides() {
  ...
  slider.forEach((slide, index) => {
    const reavealImg = slide.querySelector(".reveal-img");
    const img = slide.querySelector("img");
    const revealText = slide.querySelector(".reveal-text");
    const slideTL = gsap.timeline({
      defaults: { duration: 1, ease: "power2.inOut" },
    });
    slideTL.fromTo(reavealImg, { x: "0%" }, { x: "100%" });
    slideTL.fromTo(img, { scale: 1.6 }, { scale: 1 });
    slideTL.fromTo(revealText, { x: "0%" }, { x: "100%" }, "-=0.75");

    // Create Scene
    sliderScenes = new ScrollMagic.Scene({
  ...
  transitions: [
    {
      ...
      enter({ current, next }) {
        ...
        tl.fromTo(
          ...
          0.75,
          { x: "0" },
          { x: "100%", stagger: 0.21, onComplete: done }
        );
...
function detailAnimation() {
  ...
  slides.forEach((slide, index, slides) => {
    ...
    slideTl.fromTo(nextImg, { x: "50%" }, { x: "0%" }, "-=0.8");
    // scene
